
## 📊 Product Management Certifications

### Google Product Management – Product School

| Certification | Credential |
|---------------|------------|
| 🎯 **Product Discovery Micro-Certification (PDC)** | Product Discovery |
| 🗺️ **Product Roadmapping Micro-Certification (PRC)** | Product Roadmapping |
| ⚖️ **Product Prioritization Micro-Certification (PPC)** | Product Prioritization |
| 🧪 **Product Experimentation Micro-Certification (PEC)** | Product Experimentation |
| 📈 **Product Strategy Micro-Certification (PSC)** | Product Strategy |
| 🤖 **Artificial Intelligence Micro-Certification (AIC)** | AI for Product Managers |
| 📊 **Product Analytics Micro-Certification (PAC)** | Product Analytics |

---

### 📋 Certification Details

#### Product Discovery Micro-Certification (PDC)
- Customer problem identification and validation
- User research methodologies
- Market opportunity assessment
- Solution hypothesis formulation

#### Product Roadmapping Micro-Certification (PRC)
- Strategic roadmap development
- Outcome-based planning
- Stakeholder alignment
- Roadmap visualization and communication

#### Product Prioritization Micro-Certification (PPC)
- Prioritization frameworks (RICE, MoSCoW, Kano)
- Trade-off analysis
- Resource allocation strategies
- Data-driven decision making

#### Product Experimentation Micro-Certification (PEC)
- A/B testing design and implementation
- Experimentation best practices
- Results analysis and interpretation
- Iterative product improvement

#### Product Strategy Micro-Certification (PSC)
- Product vision and mission development
- Competitive analysis
- Go-to-market strategy
- Business model innovation

#### Artificial Intelligence Micro-Certification (AIC)
- AI/ML fundamentals for product managers
- Ethical AI considerations
- AI product development lifecycle
- Integrating AI features into products

#### Product Analytics Micro-Certification (PAC)
- Metrics framework development
- Data analysis and visualization
- User behavior tracking
- Conversion funnel optimization

---

### 🏆 Skills Demonstrated

Through these certifications, I have demonstrated proficiency in:

| Skill Area | Competencies |
|------------|--------------|
| **User-Centric Design** | Customer discovery, problem validation, user research |
| **Strategic Planning** | Roadmapping, product vision, go-to-market strategy |
| **Data-Driven Decision Making** | Experimentation, analytics, prioritization frameworks |
| **Technical Integration** | AI/ML product features, ethical AI implementation |
| **Cross-Functional Leadership** | Stakeholder management, team alignment, communication |

---

## 🎤 Toastmasters International
- Toastmasters International: Dynamic Leadership Pathway
- Toastmasters International: Visionary Communication Pathways

---

### 🔗 Additional Resources

- [Product School Official Website](https://productschool.com/)
- [Google Product Management Certification Path](https://drive.google.com/drive/folders/1f_jgCI3abxJuPQqwsejfa1IgriBKtGbB?usp=sharing)
- [My LinkedIn Certifications Section](https://www.linkedin.com/in/abigail-oyekola/details/certifications/)

---

*These micro-certifications represent specialized training in modern product management methodologies, complementing my technical engineering background with product leadership skills.*



