---
layout: page
title: Media Highlights
permalink: /media/
---

# 📰 Media Highlights

## 2025 Features

### Howard University Graduate School

| Publication | Title | Link |
|-------------|-------|------|
| 📢 What's Trending | "What's Trending | The Graduate School" | [View Article](#) |
| 📰 The Sway | "The Sway | September 2025" | [View Article](#) |
| 🏆 Bouchet Honor Society | "Ten Howard University Ph.D. Students Inducted into Bouchet Graduate Honor Society" | [View Article](#) |
| 🎓 Bouchet Honor Society | "Edward A. Bouchet Graduate Honor Society" | [View Article](#) |

### Aspen Policy Academy – Aspen Institute

| Publication | Title | Link |
|-------------|-------|------|
| 🌟 Policy Primer | "Introducing the Spring 2025 Policy Primer Cohort" | [View Article](#) |
| 🎉 Cohort Announcement | "We are thrilled to announce the Spring 2025 Policy Primer cohort" | [View Article](#) |
| 🏙️ Smart Cities | "Smart Commutes, Smarter Cities: A Government and Business Partnership" | [View Article](#) |
| 🌆 Smart Cities | "Smart Commutes, Smarter Cities" | [View Article](#) |
| 📋 Policy Brief | "Smart Commutes, Smarter Cities: Policy Brief" | [View Article](#) |
| 📊 Operational Plan | "Smart Commutes, Smarter Cities: Operational Plan" | [View Article](#) |
| 📝 Executive Summary | "Smart Commutes, Smarter Cities: Executive Summary" | [View Article](#) |
| 🛠️ Traffic Management | "Smart Commutes, Smarter Cities: Traffic Management Tool Information" | [View Article](#) |

## 📸 Media Gallery

### Howard University Features

<div style="display: flex; flex-wrap: wrap; gap: 20px; margin-top: 20px;">
  <div style="border: 1px solid #e1e4e8; border-radius: 6px; padding: 15px; width: calc(50% - 10px);">
    <h3>🎓 Bouchet Graduate Honor Society</h3>
    <p>Ten Howard University Ph.D. Students Inducted into Bouchet Graduate Honor Society</p>
    <p><em>Howard University Graduate School, 2025</em></p>
    <a href="#" style="background: #2ea44f; color: white; padding: 8px 16px; text-decoration: none; border-radius: 6px;">Read More →</a>
  </div>
  
  <div style="border: 1px solid #e1e4e8; border-radius: 6px; padding: 15px; width: calc(50% - 10px);">
    <h3>📰 The Sway</h3>
    <p>September 2025 Feature</p>
    <p><em>Howard University Graduate School Newsletter</em></p>
    <a href="#" style="background: #2ea44f; color: white; padding: 8px 16px; text-decoration: none; border-radius: 6px;">Read More →</a>
  </div>
</div>

### Aspen Policy Academy Features

<div style="display: flex; flex-wrap: wrap; gap: 20px; margin-top: 20px;">
  <div style="border: 1px solid #e1e4e8; border-radius: 6px; padding: 15px; width: calc(50% - 10px);">
    <h3>🏙️ Smart Commutes, Smarter Cities</h3>
    <p>A Government and Business Partnership</p>
    <p><em>Aspen Policy Academy, Aspen Institute, 2025</em></p>
    <a href="#" style="background: #2ea44f; color: white; padding: 8px 16px; text-decoration: none; border-radius: 6px;">Read Policy Brief →</a>
  </div>
  
  <div style="border: 1px solid #e1e4e8; border-radius: 6px; padding: 15px; width: calc(50% - 10px);">
    <h3>🌟 Spring 2025 Policy Primer Cohort</h3>
    <p>Introducing the Spring 2025 Cohort</p>
    <p><em>Aspen Policy Academy, Aspen Institute, 2025</em></p>
    <a href="#" style="background: #2ea44f; color: white; padding: 8px 16px; text-decoration: none; border-radius: 6px;">Read Announcement →</a>
  </div>
</div>

## 📊 Media Highlights by Category

### 🏛️ Policy & Urban Innovation
- *Smart Commutes, Smarter Cities* Series (8 publications) – Aspen Policy Academy
  - Government and Business Partnership
  - Policy Brief
  - Operational Plan
  - Executive Summary
  - Traffic Management Tool Information

### 🎓 Academic Recognition
- Bouchet Graduate Honor Society Inductions – Howard University
- Graduate School Features: "What's Trending" & "The Sway"

### 🌟 Policy Leadership
- Spring 2025 Policy Primer Cohort Selection – Aspen Institute

## 🔗 Quick Links

| Source | Topic | Type |
|--------|-------|------|
| [Howard University Graduate School](https://graduate.howard.edu/) | Bouchet Honor Society | Academic Recognition |
| [Aspen Policy Academy](https://www.aspeninstitute.org/) | Smart Cities Initiative | Policy Research |
| [Aspen Institute](https://www.aspeninstitute.org/) | Policy Primer Cohort | Leadership Program |

---

*Links to specific articles will be added as they become available online. Many of these features may be internal university or institute publications.*




